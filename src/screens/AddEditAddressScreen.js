        ...
        {({ handleChange, handleBlur, handleSubmit, values, errors, touched, setFieldValue }) => (
          <ScrollView contentContainerStyle={styles.formContainer} keyboardShouldPersistTaps="handled">
            ...
            <View style={styles.inputGroup}>
                ...
                {/* FIX 1: Applied placeholderTextColor to all inputs */}
                <TextInput
                    style={styles.input}
                    placeholder="House No, Building, Street, Area"
             ...
             <View style={styles.inputGroup}>
                ...
                <View style={styles.addressTypeContainer}>
                    {['Home', 'Work', 'Other'].map(type => (
                        <TouchableOpacity 
                            key={type}
                            // FIX 2: Updated logic is in the styles.addressTypeButton below
                            style={[styles.addressTypeButton, values.addressType === type && styles.addressTypeSelected]}
                            onPress={() => setFieldValue('addressType', type)}
                        >
                            <Text style={[styles.addressTypeText, values.addressType === type && styles.addressTypeSelectedText]}>{type}</Text>
...
const styles = StyleSheet.create({
    ...
    // Added color: '#000' here so your TYPED text is also black (not white)
    input: { backgroundColor: '#f8f9fa', paddingHorizontal: 15, paddingVertical: 12, borderRadius: 8, fontSize: 16, borderWidth: 1, borderColor: '#dee2e6', marginBottom: 5, color: '#000' },
